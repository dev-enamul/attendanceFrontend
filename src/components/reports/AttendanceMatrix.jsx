...
import { exportMatrixToPDF, exportMatrixToExcel } from '../../utils/exportUtils';
...
export const AttendanceMatrix = () => {
  ...
  const fetchMatrix = async (params = {}) => {
    try {
      setLoading(true);
      const response = await reportsApi.getMonthlyAttendanceMatrix({ ...filters, ...params });
      if (response.success) {
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      ...
      case '🏖️':
        return { icon: '🏖️', color: 'bg-blue-100 text-blue-800', tooltip: 'Leave' };
      case '🕐':
    ...
    <div className="space-y-4 lg:space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm lg:text-base">Visual representation of monthly attendance patterns</p>
        </div>
        ...
        <div className="p-4 lg:p-6 border-b border-gray-200">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-4 text-xs lg:text-sm">
              ...
              <span className="flex items-center space-x-1">
                <span className="w-4 h-4 bg-blue-100 rounded flex items-center justify-center text-xs">🏖️</span>
                <span>Leave</span>
              </span>
            </div>
          </div>
          ...
          <table className="w-full">
            <thead className="bg-gray-50">
              <tr>
                ...
                {dates.map((date) => (
                  <th key={date} className="px-1 lg:px-2 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[32px] lg:min-w-[40px]">
                    {date}
                  </th>
                ))}
              </tr>
